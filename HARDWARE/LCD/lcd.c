...
*	函数名称：	LCD_BGR2RGB
*
*	函数功能：	从ILI93xx读出的数据为GBR格式，而我们写入的时候
						为RGB格式。通过该函数转换
*
...
*	函数功能：	设置LCD的自动扫描方向
*
*	入口参数：	u8 dir: 0~7,代表8个方向(具体定义见lcd.h)
*
*	返回参数：	无
*
*	说明：		  注意:其他函数可能会受到此函数设置的影响
						(尤其是9341/6804这两个奇葩),所以,一般设置
						为L2R_U2D即可,如果设置为其他扫描方式,
						可能导致显示不正常.
************************************************************
*/  
...
/*
************************************************************
*	函数名称：	LCD_SSD_BackLightSet
*
*	函数功能：	SSD1963 背光设置
*
*	入口参数：	u8 pwm: 背光等级,0~100.越大越亮.
*
*	返回参数：	无
*
*	说明：		 
...
*	返回参数：	无
*
*	说明：		  该初始化函数可以初始化各种ALIENTEK出品的LCD液晶屏
						本函数占用较大flash,用户可以根据自己的实际情况,
						删掉未用到的LCD初始化代码.以节省空间.
************************************************************
*/  
void LCD_Init(void)
{ 
...
*	返回参数：	无
*
*	说明：		  区域大小:(xend-xsta+1)*(yend-ysta+1)
************************************************************
*/  
...
*	函数功能：	在指定区域内填充指定颜色块			
*
*	入口参数：	u16 (sx,sy),(ex,ey): 填充矩形对角坐标,区域大小为:(ex-sx+1)*(ey-sy+1)   
						u16 color: 要填充的颜色
*
*	返回参数：	无
*
